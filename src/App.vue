  ...
  <div id="app">
    <div class="toolbar" role="banner">
      ...
      <span>
        Eşini Bul Oyunu by
        <a
          style="color: whitesmoke; font-weight: bolder;"
          href="http://ahmettinastepe.com"
          target="_blank"
        >Ahmet Tınastepe</a>
      </span>
    ...
    <div class="content" role="main">
      ...
      <div class="card highlight-card card-small">
        ...
        <span>
          Eşini Bul Oyunu |
          <b @click="newGame(true)" style="cursor: pointer;">Yeni Oyun</b>
        </span>
      ...
      <Level v-if="isLevelShow" @initial="initialBoard($event)" />

      <GameBoard
        v-if="isGameBoardShow"
        :grouped="grouped"
        :btnLenght="randomButtons.length"
...
export default {
  ...
  methods: {
    initialBoard(level) {
      const total = level.horizontal * level.vertical;

      this.initialButtons(total);
      this.randombutton(total);
      this.random(level.horizontal);

      this.isGameBoardShow = true;
      this.isLevelShow = false;
    },

    initialButtons(total) {
      for (let i = 1; i <= total / 2; i++) {
        let btn = new Button(i);
